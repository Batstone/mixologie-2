...
export default function DrinkPage({ params }: DrinkPageProps) {
  ...
  function handleFavoriteChange(name: string, id: string) {
    ...
    let currentFavorites: { [key: string]: LocalStorageDrink } = JSON.parse(localStorage.getItem(FAVORITE_DRINKS) || "{}");
    let text;
    ...
    if (isFavorite) {
      ...
      if (currentFavorites) {
        ...
        localStorage.setItem(FAVORITE_DRINKS, JSON.stringify(favorites));
      }
    }
              ...
              <>
                <div className={styles["drink__favorites"]}>
                  <Button
                    ...
                    onClick={(e) => handleFavoriteChange(selectedDrink.name, selectedDrink.id)}
                  >
                    <svg aria-hidden="true" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 217.408 217.408">
